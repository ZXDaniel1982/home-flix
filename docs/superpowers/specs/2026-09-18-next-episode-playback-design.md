...
- **Progress reporting:** the existing cleanup/report-stopped behavior runs before advancing (web disposal effect; Android `onDispose`), so Jellyfin still records the finished episode and marks it played.

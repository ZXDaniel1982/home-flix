...
- [x] 3.1.2 — Add dependencies: jellyfin-sdk-kotlin, Media3 ExoPlayer, Coil, Navigation Compose. *(MVP, M)*
- [x] 3.1.3 — Set up navigation graph: Routes for login, movies, detail, player. *(MVP, M)*
...
- [x] 3.2.3 — Session management: Handle token expiry, logout. *(MVP, S)*

### Epic 3.3 — Movies Browsing

- [x] 3.3.1 — Movies list screen: Fetch movies, display grid using LazyVerticalGrid. *(MVP, L)*
- [x] 3.3.2 — Movie detail screen: Show poster, metadata, play button. *(MVP, L)*
- [x] 3.3.3 — Image loading with Coil: Load posters efficiently. *(MVP, S)*

### Epic 3.4 — Video Playback

- [x] 3.4.1 — Player screen with ExoPlayer: Use direct stream URL, configure ExoPlayer. *(MVP, L)*
- [x] 3.4.2 — Playback controls: Custom controls (play/pause, seek). *(MVP, M)*
- [x] 3.4.3 — Handle playback errors: Error UI on failure. *(MVP, S)*
- [x] 3.4.4 — Resume playback: Send progress updates, resume from last position. *(Recommended, M)*
...
- [x] 3.5.3 — Continue watching row on home: Use Resume endpoint. *(Recommended, M)*
...
- [x] 5.1.2 — Document API usage: List key endpoints used by clients. *(Recommended, S)*
- [x] 5.1.3 — Create user guide: How to use web/app, add media. *(Optional, S)*
...
- [x] 5.2.2 — Docker Compose production setup: Finalize compose with all services, env vars. *(Recommended, M)*
...
- [x] 5.3.2 — Media backup strategy: Document rsync or cron job. *(Optional, M)*

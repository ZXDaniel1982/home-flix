...
test('a movie shows no Episodes button', async ({ page }) => {
	await mockJellyfin(page);
	await page.route(
		(url) => /\/api\/Videos\/[^/]+\/stream$/.test(url.pathname),
		// Leave the media request pending so the control bar renders instead of
		// the playback-error message.
		() => {}
	);
	...
	await expect(page.getByRole('button', { name: 'Episodes' })).toHaveCount(0);
});

test('episode list shows an error and Retry when the fetch fails', async ({ page }) => {
	await page.route(
		(url) => url.pathname.startsWith('/api/'),
		(route) => {
			const request = route.request();
	...
	const dialog = page.getByRole('dialog', { name: 'Episodes' });
	await expect(dialog).toBeVisible();
	await expect(dialog.getByText('Could not load episodes.')).toBeVisible();

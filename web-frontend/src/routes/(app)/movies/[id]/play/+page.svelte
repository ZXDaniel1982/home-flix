<script lang="ts">
	...
	async function load(movieId: string) {
		const [info, item] = await Promise.all([getPlaybackInfo(movieId), getItem(movieId)]);
		...
		return {
			src: streamUrl(movieId, source.Id),
			msId: source.Id,
	...
	function onPlay() {
		reportPlaybackStarted(id, mediaSourceId, currentTicks()).catch(() => {});
	}
	...
	function sendProgress() {
		...
		reportPlaybackProgress(id, mediaSourceId, currentTicks(), video?.paused ?? true).catch(
			() => {}
		);
	}
	...
	$effect(() => {
		...
		return () => {
			...
			if (mediaSourceId) {
				reportPlaybackStopped(id, mediaSourceId, ticks).catch(() => {});
			}
		};
	});
</script>
